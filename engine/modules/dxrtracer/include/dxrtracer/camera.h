...
namespace dxray
{
	...
	struct Camera
	{
		...
		Camera(const fp32 a_fov, const fp32 a_aspectRatio, const fp32 a_zNear = 0.1f, const fp32 a_zFar = 1000.0f) :
			...
			ZFar(a_zFar)
		{
			m_perspective = vath::Perspective(Fov, AspectRatio, ZNear, ZFar);
		}
		...
		{
			Position = a_position;
			m_view = vath::Transpose(vath::LookAtRH(a_position, a_lookat, a_worldUp));
		}
		...
		inline const vath::Matrix4x4f GetViewProjectionMatrix() const
		{
			return Inverse(m_view * m_perspective);
		}

	private:
		vath::Matrix4x4f m_view;
		vath::Matrix4x4f m_perspective;
	};
}